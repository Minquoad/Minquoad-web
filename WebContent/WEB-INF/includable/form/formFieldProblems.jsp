...
<c:if test="${ pageScope.form.submitted }">

	<c:set var="fieldValueProblems"
		value="${ pageScope.form.getFieldValueProblems(param.fieldName) }"
		scope="page" />

	<c:if test="${ fn:length(pageScope.fieldValueProblems) ne 0 }">
		<ul class="formErrors">
			<c:forEach items="${ pageScope.fieldValueProblems }" var="problem"
				varStatus="status">
				<li>
					<c:out value="${ problem }" />
				</li>
